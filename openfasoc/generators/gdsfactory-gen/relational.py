...
class ImportCell(GlayoutAction):
    ...
    def __init__(self, component_identifiers: list[str], component_name: str, module_path: Optional[Union[str, Path]]=None, overwrite_module: bool=True):
        ...
        if module_path is None:
            ...
            if len(matching_files)>0:
                glayout_path = matching_files[0].resolve()
            else:
        ...
        if any(ind in Path(module_path).as_posix() for ind in ["/",".py"]):
            ...
            # if the module is not in the glayout directory, it should be copied into current directory so it can be imported
            module_path_parts = [part for part in module_path.parts]
            if "glayout" in module_path_parts:
                module_path = ".".join(module_path_parts[module_path_parts.index("glayout"):]).removesuffix(".py")
            else:
    ...
    def test(cls):
        tests = list()
        tests.append([["nmos","nfet"],"nmos","glayout.primitives.fet"])
        tests.append([["pmos","pfet"],"pmos","glayout.primitives.fet"])
        tests.append([["guardring","tapring","welltap","well tap", "tap ring"],"tapring","glayout.primitives.guardring"])
        tests.append([["mimcap"],"mimcap","glayout.primitives.mimcap"])
        tests.append([["mimcap array","mimcaparray","mimcap_array"],"mimcap_array","glayout.primitives.mimcap"])
        tests.append([["via","via stack","via_stack"],"via_stack","glayout.primitives.via_gen"])
        tests.append([["via array","via_array"],"via_array","glayout.primitives.via_gen"])
        tests.append([["nlp"],"test_stream","../interpreter/deprecated/../deprecated/practice_stream.py"])
        tests.append([["nmos","nfet"],"nmos","./glayout/primitives/fet.py"])
...
class GlayoutCode(GlayoutAction):
    ...
    def __init__(self, toplvl_name: str):
        ...
        self.update_import_table(["mimcap"],"mimcap","glayout.primitives.mimcap")
